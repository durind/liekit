...
switch (option) {
  ...
  Fire2012WithPalette(0); // run simulation frame, using palette colors
  Fire2012WithPalette(1);
...
void Fire2012WithPalette(int set)
{
  ...
  // Step 1.  Cool down every cell a little
    ...
    for( int j = 0; j < NUM_LEDS; j++) {
      ...
      if( gReverseDirection ) {
        pixelnumber = (NUM_LEDS+block-1) - j;
      } else {
        pixelnumber = j+block;
      }
